...
class Utils {
   ...
   * @returns {Array}
   */
  getNeedCreateModuleList(pageMap, cacheList) {
    let result = [];
    ...
    for (let key in pageMap) {
      ...
      for (let cacheObj of cacheList) {

        if (cacheObj.moduleType === 'view') {
          result.push(config.namespace + '.view.' + viewportSec + '.' + cacheObj.fileName);
        } else {
          result.push(config.namespace + '.'+ cacheObj.moduleType +'.' + moduleObj.controller);
        }
      }
    }
