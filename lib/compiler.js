...
class Compiler {
  ...
  getModuleName(templateStr = '') {
    let moduleNameRegex = /Ext\.define\(["']([a-zA-Z.]+)["']/g;

    return moduleNameRegex.exec(templateStr)[1];
  }
  ...
  getCompileNeed(moduleName) {
    ...
    let storeSectionList = sectionList.slice(0, sectionList.length);

    storeSectionList[storeSectionList.length - 1] = utils.getBigCamelCase(storeSectionList[storeSectionList.length - 1]);

    let storeSectionStr = storeSectionList.join('.');
  ...
  compile(templateStr = '') {
    ...
    templateStr = templateStr.replace(eval('/' + storeSectionStr + '/g'), ($) => {  // 替换grid配置中的store字段
      return '${storeSection}';
    });
  ...
  render(compiledStr = '', moduleName) {
    ...
    compiledStr = compiledStr.replace(/\$\{storeSection\}/g, storeSectionStr);  // 替换grid配置中的store字段
